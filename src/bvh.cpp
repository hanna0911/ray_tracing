    ...
    size_t start, size_t end, double time0, double time1) 
{
    ...
    box = new aabb(box->surrounding_box(box_left, box_right));
}
...
bool bvh_node::intersect(const Ray &r, Hit &rec, float t_min) {
    if (!box->hit(r, t_min, INFINITY))
        return false;

    bool hit_left = left->hit(r, t_min, INFINITY, rec);
    bool hit_right = right->hit(r, t_min, hit_left ? rec.t : INFINITY, rec);
